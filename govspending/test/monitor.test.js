...
test("resolveEffectiveNow prefers env and advances past skewed VM clocks", () => {
  const explicit = resolveEffectiveNow({
    now: "2026-07-27T01:01:00.000Z",
    previousRanAt: "2026-07-27T00:01:00.000Z",
    localNow: "2026-07-24T20:00:00.000Z",
  });
  assert.equal(explicit.source, "explicit");
  assert.equal(explicit.now.toISOString(), "2026-07-27T01:01:00.000Z");
  assert.equal(explicit.clockCorrected, false);

  const fromEnv = resolveEffectiveNow({
    previousRanAt: "2026-07-27T00:01:00.000Z",
    localNow: "2026-07-24T20:00:00.000Z",
    env: { GOVSPENDING_NOW: "2026-07-27T01:01:42.718Z" },
  });
  assert.equal(fromEnv.source, "env");
  assert.equal(fromEnv.now.toISOString(), "2026-07-27T01:01:42.718Z");
  assert.equal(fromEnv.clockCorrected, true);

  const advanced = resolveEffectiveNow({
    previousRanAt: "2026-07-27T00:01:44.135Z",
    localNow: "2026-07-24T20:08:30.000Z",
    env: {},
  });
  assert.equal(advanced.source, "previous-ran-at-plus-hour");
  assert.equal(advanced.now.toISOString(), "2026-07-27T01:01:44.135Z");
  assert.equal(advanced.clockCorrected, true);

  const local = resolveEffectiveNow({
    previousRanAt: "2026-07-27T00:01:00.000Z",
    localNow: "2026-07-27T01:05:00.000Z",
    env: {},
  });
  assert.equal(local.source, "local");
  assert.equal(local.now.toISOString(), "2026-07-27T01:05:00.000Z");
  assert.equal(local.clockCorrected, false);
});
...
test("runMonitor advances lookback window when local VM clock is behind", async () => {
  const tempDir = await mkdtemp(path.join(os.tmpdir(), "govspending-"));
  try {
    await cp(path.join(GOVSPENDING_ROOT, "config.json"), path.join(tempDir, "config.json"));
    await cp(
    ...
    await writeFile(
      path.join(tempDir, "data", "last-run.json"),
      `${JSON.stringify(
        {
          ranAt: "2026-07-27T00:01:44.135Z",
          sourceMode: "fixtures-fallback",
          admin: {
            overall: "degraded",
            degradedStreak: 13,
    ...
    const result = await runMonitor({
      ...
      // Inject a skewed local clock; no GOVSPENDING_NOW override.
      localNow: "2026-07-24T20:08:30.000Z",
      env: {},
    });

    assert.equal(result.lastRun.window.endDate, "2026-07-27");
    assert.equal(result.lastRun.ranAt, "2026-07-27T01:01:44.135Z");
    assert.equal(
      result.lastRun.admin.firstDegradedAt,
      "2026-07-26T12:04:02.442Z"
    );
    assert.equal(result.lastRun.admin.degradedStreak, 14);
    assert.equal(result.lastRun.admin.ops.backwardClockSkew, false);
    assert.equal(result.lastRun.admin.ops.clockCorrected, true);
    assert.equal(
      result.lastRun.admin.ops.clockSource,
      "previous-ran-at-plus-hour"
    );
  } finally {
    await rm(tempDir, { recursive: true, force: true });
  }
