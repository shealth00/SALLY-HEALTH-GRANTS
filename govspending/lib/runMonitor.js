 ...
 * @param {Date|string|number} [options.localNow] Injectable local clock (tests)
 * @param {NodeJS.ProcessEnv} [options.env] Injectable env (tests)
 */
export async function runMonitor(options = {}) {
  ...
  const effective = resolveEffectiveNow({
    now: options.now,
    previousRanAt: previousLastRun?.ranAt,
    localNow: options.localNow,
    env: options.env,
  });
  const monitorNow = effective.now;
  const window = computeWindow(monitorNow, config.lookbackDays);
  ...
  if (sourceMode === "fixtures" || sourceMode === "fixtures-fallback") {
    ...
    if (sourceMode === "fixtures-fallback" && previousWasLive) {
      ...
      const generatedAt = monitorNow.toISOString();
      const summary = {
      ...
      if (effective.clockCorrected) {
        admin.ops = {
          ...admin.ops,
          clockSource: effective.source,
          clockCorrected: true,
        };
      }
